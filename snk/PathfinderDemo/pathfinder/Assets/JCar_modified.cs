...
public class JCar_modified : MonoBehaviour { 
	...
	public float rotationSpeed = 1.05f;
 
    public float suspensionDistance = 1f; // amount of movement in suspension
    public float springs = 10.0f; // suspension springs
    public float dampers = 0.001f; // how much damping the suspension has
    public float wheelRadius = 0.5f; // the radius of the wheels
    public float torque = 100f; // the base power of the engine (per wheel, and before gears)
    public float brakeTorque = 100f; // the power of the braks (per wheel)
    ...
    void FixedUpdate () {
        ...
        foreach (WheelData w in wheels) {
			...
			rigidbody.AddTorque (Vector3.right * rotationSpeed * steer);
